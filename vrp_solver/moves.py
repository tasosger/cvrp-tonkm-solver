"""Local-search move/neighborhood operators shared by every improvement algorithm.

Each move class evaluates its cost delta against the ton-km objective (see
`model.Route.calculate_total_route_cost`) without materializing the resulting
route, then can `apply()` itself in place. `affected_nodes`/`affected_arcs`
are used by the tabu-search variants to build their tabu lists.
"""

import copy

from .model import EMPTY_VEHICLE_WEIGHT


class RelocationMove:
    """Move a single node from one route (position) to another."""
    ...
    def calculate_move_cost(self):
        ...
        if from_next_id != 0:
            cost_effect_from_route = (load_after_n + EMPTY_VEHICLE_WEIGHT) * (self.cost_matrix[from_prev_id][from_next_id] - self.cost_matrix[from_prev_id][self.node.id] -
                                                            self.cost_matrix[self.node.id][from_next_id]) - self.node.demand * self.from_route.prefix_distances[self.from_index]
        else:
            cost_effect_from_route = - self.node.demand * self.from_route.prefix_distances[self.from_index] - EMPTY_VEHICLE_WEIGHT * self.cost_matrix[from_prev_id][self.node.id]
        if to_next_id != 0:
            cost_effect_to_route = (load_after_n_to + EMPTY_VEHICLE_WEIGHT) * (self.cost_matrix[to_prev_id][self.node.id] + self.cost_matrix[self.node.id][to_next_id] -
            ...
            cost_effect_to_route = self.node.demand * (self.to_route.prefix_distances[self.to_index - 1] + self.cost_matrix[to_prev_id][self.node.id]) + EMPTY_VEHICLE_WEIGHT * self.cost_matrix[to_prev_id][self.node.id]
...
class SwapMove:
    """Swap two nodes that live in different routes."""
    ...
    def calculate_move_cost(self):
        ...
        cost_effect_from_route_1 = (load_after_1 + EMPTY_VEHICLE_WEIGHT) * (self.cost_matrix[prev_id_1][self.node2.id] + self.cost_matrix[self.node2.id][next_id_1] - self.cost_matrix[prev_id_1][self.node1.id] -
        ...
        cost_effect_from_route_2 = (load_after_2 + EMPTY_VEHICLE_WEIGHT) * (self.cost_matrix[prev_id_2][self.node1.id] + self.cost_matrix[self.node1.id][next_id_2] - self.cost_matrix[prev_id_2][self.node2.id] -
...
class TwoOptMove:
    """Reverse the tail of two routes and swap them (inter-route 2-opt)."""
    ...
    def calculate_move_cost(self):
        ...
        cost_route_1 = (self.route2.load - self.route2.prefix_loads[self.j - 1] + EMPTY_VEHICLE_WEIGHT) * (self.route1.prefix_distances[self.i - 1] +
                                                                                   self.cost_matrix[node_before_i.id][node_j.id]) - \
                                                                                    (self.route1.load - self.route1.prefix_loads[self.i - 1] + EMPTY_VEHICLE_WEIGHT) * \
                                                                                    (self.route1.prefix_distances[self.i])
        cost_route_2 = (self.route1.load - self.route1.prefix_loads[self.i - 1] + EMPTY_VEHICLE_WEIGHT) * (self.route2.prefix_distances[self.j - 1] +
                                                                                   self.cost_matrix[node_before_j.id][node_i.id]) - \
                                                                                    (self.route2.load - self.route2.prefix_loads[self.j - 1] + EMPTY_VEHICLE_WEIGHT) * \
...
class InRouteTwoOptMove:
    """Reverse a contiguous segment [i, j] of a single route (classic 2-opt)."""
    ...
    def calculate_move_cost(self):
        ...
        cost = (load_after_j + EMPTY_VEHICLE_WEIGHT) * (distances[node_before_i.id][node_j.id] + distances[node_i.id][node_after_j.id] - distances[node_before_i.id][node_i.id] - distances[node_j.id][node_after_j.id])
...
class InRouteSwapMove:
    ...
    def calculate_move_cost(self):
        ...
        if self.index2 == self.index1 + 1:
            cost = (self.route.load - self.route.prefix_loads[self.index2] + EMPTY_VEHICLE_WEIGHT) * (self.cost_matrix[node_before_1.id][node2.id] +
...
class InRouteReinsertMove:
    """Remove a node and reinsert it at a different position in the same route."""
