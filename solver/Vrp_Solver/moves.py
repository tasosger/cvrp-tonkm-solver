...
class RelocationMove:
    def __init__(self, node, from_route, to_route, from_index, to_index, capacity, cost_matrix):
        ...
        self.affected_routes = [from_route, to_route]
...
class SwapMove:
    ...
    def __init__(self, route1, route2, index1, index2, cost_matrix, capacity):
        ...
        self.affected_arcs = self.calculate_affected_arcs()
        self.affected_nodes = self.calculate_affected_nodes()
        self.affected_routes = [route1, route2]
    ...
    def calculate_move_cost(self):
        ...
        if next_id_1 !=0:
            ...
            cost_effect_from_route_1 = -self.node1.demand * self.route1.prefix_distances[self.index1]  + self.node2.demand*(self.route1.prefix_distances[self.index1 - 1] + self.cost_matrix[prev_id_1][self.node2.id]) +8*(self.cost_matrix[prev_id_1][self.node2.id] -self.cost_matrix[prev_id_1][self.node1.id])
        ...
        if next_id_2:
            ...
            cost_effect_from_route_2  = - self.node2.demand * self.route2.prefix_distances[self.index2] + self.node1.demand*(self.route2.prefix_distances[self.index2 - 1] + self.cost_matrix[prev_id_2][self.node1.id]) +8*(self.cost_matrix[prev_id_2][self.node1.id] -self.cost_matrix[prev_id_2][self.node2.id])
...
class InRouteReinsertMove:
    ...
    def calculate_move_cost(self):
        lenght = len(self.route.sequence_of_nodes)
        cost = 0
        if self.from_index > self.to_index and lenght > 1+ self.from_index:
            ...
            load_after_to_before_from = self.route.prefix_loads[self.from_index-1] - self.route.prefix_loads[self.to_index-1]
        ...
        elif self.from_index < self.to_index and self.to_index +1 < lenght:
            node = self.route.sequence_of_nodes[self.from_index]
            node_after_from = self.route.sequence_of_nodes[self.from_index + 1] if self.from_index + 1 < len(self.route.sequence_of_nodes) else self.route.sequence_of_nodes[0]
            cost =0
            node_before_from = self.route.sequence_of_nodes[self.from_index -1] 
            ...
            cost += node.demand * (self.cost_matrix[node_before_from.id][node_after_from.id]+ self.cost_matrix[node_before_to.id][node.id]  - self.cost_matrix[node_before_from.id][node.id] + self.route.prefix_distances[self.to_index] - self.route.prefix_distances[self.from_index+1])
            ...
            node_before_to = self.route.sequence_of_nodes[self.to_index -1]
            load_after_to_before_from = self.route.prefix_loads[self.from_index-1] - self.route.prefix_loads[self.to_index-1]
            cost = (8)* (self.cost_matrix[node_before_to.id][node.id] + self.cost_matrix[node.id][node_after_to.id] - self.cost_matrix[node_before_from.id][node.id]  - self.cost_matrix[node_before_to.id][node_after_to.id])
            cost += node.demand * (self.cost_matrix[node_before_to.id][node.id] -(self.route.prefix_distances[self.from_index] - self.route.prefix_distances[self.to_index -1]))
            cost += load_after_to_before_from * (self.cost_matrix[node_before_to.id][node.id] + (self.cost_matrix[node_after_to.id][node.id] - self.cost_matrix[node_before_to.id][node_after_to.id]))
        else:
            node = self.route.sequence_of_nodes[self.from_index]
            ...
            cost  = load_after_from_before_to * (self.cost_matrix[node_before_from.id][node_after_from.id] - self.cost_matrix[node_before_from.id][node.id] - self.cost_matrix[node.id][node_after_from.id])
            cost += (8)* (self.cost_matrix[node_before_from.id][node_after_from.id]+ self.cost_matrix[node_before_to.id][node.id] - self.cost_matrix[node_before_from.id][node.id] - self.cost_matrix[node.id][node_after_from.id])
            cost += node.demand * (self.cost_matrix[node_before_from.id][node_after_from.id]+ self.cost_matrix[node_before_to.id][node.id]  - self.cost_matrix[node_before_from.id][node.id] + self.route.prefix_distances[self.to_index] - self.route.prefix_distances[self.from_index+1])
    ...
    def apply(self):
        ...
        self.route.cost = self.route.calculate_total_route_cost(self.cost_matrix)
        self._recalculate_prefix_distances(self.route)
        self._recalculate_prefix_loads(self.route)
